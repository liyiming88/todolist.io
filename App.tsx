...
export default function App() {
  ...
  useEffect(() => {
    try {
      const savedTodos = localStorage.getItem('genius-tasker-todos');
      if (savedTodos) {
  ...
  useEffect(() => {
    localStorage.setItem('genius-tasker-todos', JSON.stringify(todos));
  }, [todos]);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 py-8 px-4 sm:px-6 lg:px-8 font-sans selection:bg-indigo-100 selection:text-indigo-900">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <footer className="text-center text-slate-400 text-sm">
          <p>© {new Date().getFullYear()} Genius Tasker. Built with React & Gemini.</p>
        </footer>
